...
namespace fe::vm
{
	executable link(program p)
	{
		...
		for (int i = 0; i < code.size();)
		{
			...
			{
				uint32_t id = read_ui32({ data[i + 1], data[i + 2], data[i + 3], data[i + 4] });
		...
		// The minimal chunk id is the number of chunks that have at least passed during iteration
		// It is used to optimize finding the chunk of a position i (used for resolving calls)
		...
		for (int i = 0; i < code.size();)
		{
			...
			{
				auto label = read_ui32(bytes<4>{data[i + 1], data[i + 2], data[i + 3], data[i + 4]});
			...
			{
				auto label = read_ui32(bytes<4>{data[i + 2], data[i + 3], data[i + 4], data[i + 5]});
				auto label_location = label_locations.at(label);
				auto offset = label_location - i;
				//assert(offset > std::numeric_limits<int32_t>::min() && offset < std::numeric_limits<int32_t>::max());
			...
			case op_kind::CALL_UI64:
			{
				...
				if (auto it = function_locations.find(function_name); it != function_locations.end())
				{
					function_location = function_locations.at(function_name);
					*(reinterpret_cast<uint64_t*>(&data[i + 1])) = function_location - i;
				}
