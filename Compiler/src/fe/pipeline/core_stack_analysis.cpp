...
namespace fe::core_ast
{
	node_id predecessor(node_id n, ast& ast)
	{
		...
		if (auto it = std::find(parent.children.begin(), parent.children.end(), node.id); it != parent.children.begin())
			return *(--it);
		else
	...
	size_t predecessor_size(node_id n, ast& ast, stack_analysis_result& res)
	{
		...
		if (auto it = std::find(parent.children.begin(), parent.children.end(), node.id); it != parent.children.begin())
		{
			return res.node_stack_sizes.at(*(--it));
		}
		else
	...
	void analyze_stack(node_id n, ast& ast, stack_analysis_result& res)
	{
		...
		case node_type::FUNCTION: { res.node_stack_sizes[n] = predecessor_size(n, ast, res); break; }
		...
		case node_type::LABEL: {
			...
			if (kind != node_type::JMP && kind != node_type::JZ && kind != node_type::JNZ)
				res.node_stack_sizes[n] = predecessor_size(n, ast, res);
			...
			ah.for_all([&res, &ast, &lbl, n](auto& jmp) {
				auto kind = jmp.kind;
				if (kind != node_type::JMP && kind != node_type::JZ && kind != node_type::JNZ) return;
				...
				if (ast.get_node_data<label>(jmp.id).id == lbl.id)
				{
					...
					if (jmp_it != end && n_it == end) res.node_stack_sizes[n] = jmp_it->second;
					// Both no size
					if (jmp_it == end && n_it == end) { assert(!"nyi"); }
					// Label has size, jmp does not
					if (jmp_it == end && n_it != end) res.node_stack_sizes[jmp.id] = n_it->second;
		...
		default: {
			if (is_binary_op(node.kind)) {
				...
				case core_ast::node_type::OR:
					res.node_stack_sizes[n] = res.node_stack_sizes[ast.get_node(n).children[0]];
					break;
