...
{
	ast::node<symbol>* parser::parse(const std::string& initial, std::vector<terminal> input) const
	{
		input.push_back(end_of_input);

		std::stack<ast::node<symbol>*> stack;
		ast::node<symbol>* root = new ast::node<symbol>(rules.to_symbol(initial));
		stack.push(root);
	...
	void parser::parse(std::stack<ast::node<symbol>*>& stack, std::vector<terminal>& input) const
	{
		...
		while (!stack.empty())
		{
			...
			else
			{
				...
				if (rules.mapping.find(non_terminal_symbol) == rules.mapping.end()) 
					throw std::runtime_error(std::string("Unable to unify, unrecognized non-terminal: ").append(std::to_string((int)input_token)));

				auto symbol_rules = rules.mapping.equal_range(non_terminal_symbol);

				// Find a rule that matches
				auto rule_location = std::find_if(symbol_rules.first, symbol_rules.second, [&](auto& rule) { return rule.second.at(0).matches(input_token, rules.mapping); });
				if (rule_location == symbol_rules.second)
					throw std::runtime_error(std::string("Unable to unify, no matching rule: ").append(std::to_string((int)input_token)));
				auto rule_rhs = rule_location->second;
